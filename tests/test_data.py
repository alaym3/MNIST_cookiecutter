...
def test_data_sample_sizes():
    ...
    assert len(train_set) == 40000, "Training dataset did not have the correct number of samples"
    assert len(test_set) == 5000, "Testing dataset did not have the correct number of samples"
    # assert shape
...
def test_labels():
    ...
    torch.unique(train_set.targets) == 9, "The targets do not have 9 unique values as required."

train_set, test_set = load_data()
@pytest.mark.parametrize("test_input,expected", [("len(train_set)", 40000), ("len(test_set)", 5000), \
    ("train_set[0][0].shape", torch.Size([1,28,28])), ("test_set[0][0].shape", torch.Size([1,28,28]))])
